...
export const GET = async (
  ...
  try {
    ...
    const { data, status } = await useFetch(
      `https://otakudesu.cloud/anime/${id}`
    );
    ...
    const $parentListLink = $(
      "#venkonten > div.venser > div:nth-child(8) > ul > li"
    );
    const title = $("#venkonten > div.venser > div.jdlrx > h1")
      .text()
      .replace("Subtitle Indonesia", "")
      .trim();
    const thumbnail = $("#venkonten > div.venser > div.fotoanime > img").attr(
      "src"
    );
    ...
    return NextResponse.json(
      { status: 200, succes: true, data: responseData },
      { status: 200 }
    );
  } catch (error: any) {
