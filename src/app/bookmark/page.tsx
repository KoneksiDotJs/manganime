  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (storedBookmarks) {
        ...
        Promise.all(
          parsedBookmarks.map((slug: string) => axios.get(`/api/anime/${slug}`))
        ).then((responses) => {
      ...
      {bookmarks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {bookmarks.map((bookmark: any, index: number) => (
            <div
              className="card p-0 card-side bg-gray-800 shadow-xl min-h-fit clickAnimation-list"
              key={index}
            >
              <div className="w-full p-2 card-body">
                ...
                <a
                  href={`/${bookmark.slug}`}
                  className="bg-zinc-800 text-white text-sm p-2 overflow-hidden rounded-md block"
                >
                  <div className="flex items-center mb-2">
                    ...
                    <div className="w-10 p-2 card-body">
                      ...
                      {bookmark.info &&
                        bookmark.info.find((info: string) =>
                          info.startsWith("Genre:")
                        ) && (
                          <p className="text-white text-sm mt-2">
                            Genre:{" "}
                            {bookmark.info
                              .find((info: string) => info.startsWith("Genre:"))
                              ?.replace("Genre: ", "")}
                          </p>
                        )}
